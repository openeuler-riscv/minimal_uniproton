 ...
 * 模块内宏定义
 */
#define OS_MEM_ADDR_ALIGN_TYPE_TO_SIZE(size) (0x1UL << (uint32_t)(size))

/* 申请一个内存块 */
typedef void *(*MemAllocFunc)(uint32_t size);

/* 申请size字节并返回指向已分配内存的指针，内存地址将按照alignPow动态对齐 */
typedef void *(*MemAllocAlignFunc)(uint32_t size, enum MemAlign alignPow);

/* 释放一个内存块  */
typedef uint32_t (*MemFreeFunc)(void *addr);
