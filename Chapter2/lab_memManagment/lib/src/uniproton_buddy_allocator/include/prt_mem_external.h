...
#define OS_FSC_MEM_SZGET(currBlk)                           ((uint32_t)(currBlk->size))
...
extern void *OsMemAlloc(uint32_t size);
extern void *OsMemAllocAlign(uint32_t size, enum MemAlign alignPow);

/* 对齐之后，返回地址不一定紧跟在内存头后面，需要设置返回地址与内存头之间的差值 */
static inline void OsMemSetHeadAddr(uintptr_t usrAddr, uintptr_t ctrlAddr)
{
    uint16_t *headAddr = (uint16_t *)(usrAddr) - 1;

    *headAddr  = (uint16_t)(usrAddr - ctrlAddr);
    return;
}

static inline void *OsMemGetHeadAddr(uintptr_t usrAddr)
{
    uint16_t headOffset = *((uint16_t *)usrAddr - 1);
